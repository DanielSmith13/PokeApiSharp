...
public class PokeApiCacheTests
{
    ...
    [Fact]
    public void Cache_RespectsCacheDuration_WhenValueIsProvided()
    {
        var fakeCache = new MockMemoryCache();
        var cache = new PokeApiCache(fakeCache, TimeSpan.FromSeconds(1));

        cache.SetCachedResource("url", "value");
        Assert.Equal(TimeSpan.FromSeconds(1), fakeCache.LastAbsoluteExpirationRelativeToNow);
    }

    [Fact]
    public void Cache_UsesDefaultCacheDuration_WhenNoDurationIsProvided()
    {
        var fakeCache = new MockMemoryCache();
        var cache = new PokeApiCache(fakeCache);

        cache.SetCachedResource("url", "value");
        Assert.Equal(TimeSpan.FromHours(1), fakeCache.LastAbsoluteExpirationRelativeToNow);
    }
}